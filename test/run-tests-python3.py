...
class ResponseHookImporter(object):
    def __init__(self, www_path):
        ...
        if 'test_www' not in sys.modules:
            spec = importlib.util.spec_from_file_location('test_www', www_path + '/__init__.py')
            module = importlib.util.module_from_spec(spec)
            sys.modules['test_www'] = module
...
class TAPTestGroup(TestGroup):
    ...
    diag_r = re.compile(rb"^#(#*)\s*(.*)$")
    plan_r = re.compile(rb"^1..(\d+)(?:\s*\#\s*SKIP(?::\s*(.*)))?$")
    test_r = re.compile(rb"^(not ok|ok)\s*"
                        rb"([0-9]+)?\s*"
                        rb"([^#]*)(?:# (TODO|SKIP))?$")
...
class TestRunner(object):
    ...
    def prepare_environ(self):
        ...
        # Run all the tests in the "C" locale.  (A Utf8-based locale
